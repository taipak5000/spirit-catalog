...
- `spirit-tree-data.js` — 本体データ。`const SPIRIT_TREE_DATA = [...]`(216件、うち213件に日本語精霊名`nameJa`つき)
...
- Season(季節)精霊: 169体中169体
- Regular(恒常)精霊: 37体中37体(realm=王国ごとの照合、6王国すべて一致確認済み)
- Elder(長老)精霊: 6体中6体
- Guide(季節ガイドNPC)精霊: 30体中30体
  - うち23体は「(シーズン名)の案内人」という定型パターンで命名
  - 5体(壁画の精・砕ける闇の虚・バラ・ムーミンの本・希望の番人)は個別にWeb調査して確認した固有名
  - 残り2体(ふたつの灯火の季節・ゴッホの季節のガイド)は固有名を確認できなかったため、
    定型パターン(「(シーズン名)の案内人」)を代わりに使っています
- Special(特殊)精霊: 4体中1体(「思いやり深いチェリスト」のみ判明)
- 合計 **213/216** 体に日本語名あり(残り3体は「巣づくりの挑戦」系の特殊精霊)
...
2,343ノード中 **2,089件(89%)** に日本語名がついています。以下の順で突き合わせました。

1. 汎用アイテムの手作業辞書(ハート・祝福・ウィングバフ・クエストN・染料・Stanceポーズ・動物の鳴き声・楽器名など)
...
**注意**: 「Music」種別(楽曲・BGM名、約55件)の日本語名は、既存サイトや外部資料との突き合わせではなく、
**アシスタントによる直訳(未検証)** です。公式・コミュニティでの呼び方と異なる可能性があります。

未解決の254件(11%)は主に: 長老の「アルティメット」報酬(itemのcost-data.jsに未収録)、
精霊名(nameJa)が未確認の3体(「巣づくりの挑戦」系)分のアイテムです。

## 現時点でわかっている制限・未対応

- 精霊名(nameJa)が未確認なのは3体: 「巣づくりの挑戦」系の特殊精霊(Workshop/Nest/Friends)。
...
2. (任意)「Music」種別の直訳を、正式名称・コミュニティでの呼称と突き合わせて精度を上げる
3. (任意)長老の「アルティメット」報酬・「巣づくりの挑戦」3体分の追加調査
